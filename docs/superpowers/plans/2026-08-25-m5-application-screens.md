...
**Carried-forward risks:** Recharts does not read CSS custom properties on its own, so Task 8 has to pass resolved colours or dark mode keeps the light palette — the same class of bug as M4's interpolated status classes. The `/jobs` redirect is a live URL change on a deployed app, so Task 2's build must be checked in the route table, not just locally. And `src/screens/__tests__/ResumePage.test.ts` hardcodes the forbidden Supabase project ref; Task 7 deletes `ResumePage` but **not** that test file — clean it up there rather than leaving an orphan.
