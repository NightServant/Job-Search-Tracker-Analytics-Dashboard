...
function ResumeHub({ drafts, isLoading, onOpenDraft, onCreateNew, onDeleteDraft }: { drafts: ResumeDraft[]; isLoading: boolean; onOpenDraft: (draft: ResumeDraft) => void; onCreateNew: () => void; onDeleteDraft: (draftId: string) => void }) {
  return <section className="card p-5 md:p-6"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h1 className="text-2xl font-bold text-zinc-900 dark:text-white">CV Builder</h1><p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Open an existing draft or start a new one in Word or LaTeX mode.</p></div><button className="btn-primary" onClick={onCreateNew}><Plus className="w-4 h-4" />Add New CV</button></div><div className="mt-5 space-y-3">{isLoading ? <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 p-6 text-center text-zinc-500 dark:text-zinc-400">Loading your drafts...</div> : drafts.length === 0 ? <div className="rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/40 p-8 text-center"><p className="text-lg font-semibold text-zinc-900 dark:text-white">No drafts yet</p><p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Create your first CV draft, then choose Word or LaTeX.</p><button className="btn-primary mt-4" onClick={onCreateNew}><Plus className="w-4 h-4" />Add New CV</button></div> : drafts.map((draft) => <article key={draft.id} className="flex flex-col gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-4 md:flex-row md:items-center md:justify-between"><button type="button" className="text-left flex-1" onClick={() => onOpenDraft(draft)}><div className="flex items-center gap-2"><p className="font-semibold text-zinc-900 dark:text-white">{draft.title}</p><DraftModePill mode={draft.mode} /></div><p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Updated {new Date(draft.updated_at).toLocaleString()}</p></button><div className="flex items-center gap-2"><button type="button" className="btn-secondary" onClick={() => onOpenDraft(draft)}>Open</button><button type="button" className="rounded-lg border border-zinc-200 dark:border-zinc-800 p-2 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900" title="Delete draft" onClick={() => onDeleteDraft(draft.id)}><Trash2 className="w-4 h-4" /></button></div></article>)}</div></section>
}
...
function WordResumeEditor({ draft, onBack, onDelete, onPersistDraft }: { draft: ResumeDraft; onBack: () => void; onDelete: (draftId: string) => void; onPersistDraft: (draftId: string, title: string, mode: ResumeMode, content: ResumeContent) => Promise<ResumeDraft> }) {
  ...
  const saveDraft = async (notify = false) => {
    ...
    setIsSaving(true)
    try {
      const updated = await onPersistDraft(draft.id, title.trim() || 'Untitled CV', 'word', editor.getJSON())
  ...
  useEffect(() => {
    ...
    snapshotTimerRef.current = window.setTimeout(() => { void createSnapshotForAutosave() }, 5000)
    return () => { if (snapshotTimerRef.current) window.clearTimeout(snapshotTimerRef.current) }
  }, [isDirty, editor, user])
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      const safeName = (title.trim() || 'cv').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
  ...
  const applyTemplate = (template: ResumeTemplate) => {
    if (template.mode !== 'word' || (template.content as any).type === 'latex') {
  ...
  const restoreSnapshot = async (content: any) => {
...
function LatexResumeEditor({ draft, onBack, onDelete, onPersistDraft }: { draft: ResumeDraft; onBack: () => void; onDelete: (draftId: string) => void; onPersistDraft: (draftId: string, title: string, mode: ResumeMode, content: ResumeContent) => Promise<ResumeDraft> }) {
  ...
  const saveDraft = async (notify = false) => {
    setIsSaving(true)
    try {
      const updated = await onPersistDraft(draft.id, title.trim() || 'Untitled CV', 'latex', { type: 'latex', source: latexSource })
      setLastSavedAt(updated.updated_at)
      setIsDirty(false)
  ...
  const applyTemplate = (template: ResumeTemplate) => {
    if (template.mode !== 'latex' || (template.content as any).type !== 'latex') {
      showError('Invalid template', 'Cannot apply Word template to LaTeX editor')
      return
    }
    const source = (template.content as any).source
  ...
  const restoreSnapshot = async (content: any) => {
    if (content && typeof content === 'object' && (content as any).type === 'latex') {
      const source = (content as any).source
      setLatexSource(source)
      setIsDirty(true)
      await saveDraft(false)
    }
  ...
  return <section className="card p-5 md:p-6 space-y-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><button className="btn-secondary" onClick={onBack}><ArrowLeft className="w-4 h-4" />Back to drafts</button><div><h1 className="text-2xl font-bold text-zinc-900 dark:text-white">LaTeX CV</h1><p className="text-sm text-zinc-500 dark:text-zinc-400">Source editor with live preview{cdnAvailable ? ' ✓' : ' (rendering optimized)'}.</p></div></div><div className="flex flex-wrap items-center gap-2">{user && <ResumeVersionHistory resumeId={draft.id} userId={user.id} onRestore={restoreSnapshot} />}<TemplatePresetSelector mode="latex" onSelect={applyTemplate} /><button className="btn-secondary" onClick={resetTemplate}><RotateCcw className="w-4 h-4" />Reset</button><button className="btn-secondary" onClick={() => void saveDraft(true)} disabled={isSaving}><Save className="w-4 h-4" />{isSaving ? 'Saving...' : 'Save'}</button><button className="btn-secondary" onClick={copyLatex}><Code2 className="w-4 h-4" />Copy LaTeX</button><button type="button" className="rounded-lg border border-zinc-200 dark:border-zinc-800 p-2 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900" onClick={() => onDelete(draft.id)} title="Delete draft"><Trash2 className="w-4 h-4" /></button></div></div><div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_auto] md:items-center"><label className="block"><span className="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">CV title</span><input value={title} onChange={(e) => { setTitle(e.target.value); setIsDirty(true) }} placeholder="Untitled CV" className="mt-1 w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-primary-500" /></label><div className="text-sm text-zinc-500 dark:text-zinc-400 md:text-right">{formatSaveTime(lastSavedAt)}{isDirty ? <span className="ml-2 text-amber-500">Unsaved changes</span> : null}</div></div><div className="grid grid-cols-1 gap-4 xl:grid-cols-2"><div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/60 dark:bg-zinc-900/40 p-3"><div className="mb-2 flex items-center justify-between"><p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">LaTeX Source</p><span className="text-xs text-zinc-500">{latexSource.length} chars</span></div><textarea value={latexSource} onChange={(e) => { setLatexSource(e.target.value); setIsDirty(true) }} className="h-[540px] w-full resize-y rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 px-3 py-2 font-mono text-xs text-zinc-800 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-primary-500" spellCheck={false} aria-label="LaTeX source" /></div><div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/60 dark:bg-zinc-900/40 p-3"><div className="mb-2 flex items-center justify-between"><p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Preview</p><span className="inline-flex items-center gap-1 text-xs text-zinc-500"><FileText className="w-3.5 h-3.5" />{cdnAvailable ? 'Live rendering' : 'Readable preview'}</span></div><iframe title="LaTeX preview" srcDoc={previewHtml} className="h-[540px] w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white" sandbox="allow-scripts allow-same-origin" /></div></div></section>
}
...
export default function ResumePage() {
  ...
  const persistDraft = async (draftId: string, title: string, mode: ResumeMode, content: ResumeContent): Promise<ResumeDraft> => {
    ...
    if (error) throw error
    const updatedDraft: ResumeDraft = { id: data.id, title: data.title || 'Untitled CV', mode: normalizeMode(data.mode), content: data.content as ResumeContent | null, updated_at: data.updated_at }
  ...
  const createDraft = async (mode: ResumeMode) => {
    ...
    try {
      ...
      const defaultTitle = mode === 'latex' ? 'Untitled LaTeX CV' : 'Untitled CV'
      ...
      {loadError && (
        <div className="card p-6 border-l-4 border-red-500 bg-red-50 dark:bg-red-950">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-900 dark:text-red-100">Could not load CVs</h3>
